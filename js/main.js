...
function appendRecipeCard(recipe, idx, recipeContainer) {
  let overlapping = fridgeChoice.filter((fridgeItem) =>
    recipe.ingredients.includes(fridgeItem)
  );
  ...
  newRecipeCard.innerHTML = `
	<div class="collapsible-header">
		<img width="50" height="50" id="${recipe.title.toLowerCase()}" src=${
    recipe.thumbnail
  }>
		<div class='collapsible-text'>
			<h1 class="recipe-title">${recipe.title}</h1>
			<h2 class="recipe-overlap">You have ${overlapping.length} out of ${
    recipe.ingredients.length
  } ingredients for this recipe.</h2>
		</div>
	</div>
  ...
  createShoppingList.addEventListener("click", (e) =>
    appendToShoppingList(recipe)
  );
  appendUl(idx);
...
function checkFridgeIncludes(ingredient, className, li) {
  fridgeChoice.includes(ingredient)
    ? li.setAttribute("class", `${className} exists`)
    : li.setAttribute("class", `${className} empty`);
}
